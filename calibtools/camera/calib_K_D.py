...
LastEditors: fanjin 
LastEditTime: 2024-07-21 08:52:57
...
def calib_camera_charuco():
    ...
    if cam_model == CamModel.FISH:
        ...
        flags = cv2.CALIB_FIX_K4 | cv2.CALIB_FIX_K5 | cv2.CALIB_FIX_K6 | cv2.CALIB_FIX_S1_S2_S3_S4 | cv2.CALIB_FIX_TAUX_TAUY
        criteria = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 100, 1e-9)
        retval, K, D, rvecs, tvecs = aruco.calibrateCameraCharuco(
            all_corners, all_ids, board, image_size, None, None, None, None, flags=flags, criteria=criteria)
    print(retval)
    #
    calib_res = {}
    ...
    for i, obj_points in enumerate(all_obj_points):
        ...
        if cam_model == CamModel.FISH:
            ...
            img_undist = cv2.fisheye.undistortImage(img, K, D, None, K, (img.shape[1], img.shape[0]))
        else:
        ...
        if enable_save_intermediate_result:
            ...
            cv2.imwrite(f'{output_dir}/temp/un_{os.path.basename(all_paths[i])}.jpg', img_undist)
...
def main():
    ...
    # 模拟命令行参数
    args = [
        '-i', '/home/frank/data/G3/scan_calib/20240718/side2/back',
        # '-i', '/home/frank/data/G3/scan_calib/20240718/side4/lf',
        # '-i', '/home/frank/data/G3/scan_calib/20240718/lb-rf',
        '-c', '/home/frank/data/GitHub/calibtools/calibtools/camera/charuco.yaml',
        '-o', '/home/frank/data/GitHub/calibtools/data/calib_results/side-b',
        '-s',
        # '-m', 'pinhole'
        '-m', 'fisheye'
    ]
