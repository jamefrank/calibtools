...
LastEditors: fanjin 
LastEditTime: 2024-07-25 16:20:22
...
from calibtools.file_utils import np_to_cv_yaml
...
from calibtools.thirdparty.SuperGluePretrainedNetwork.models.matching import Matching
from calibtools.thirdparty.SuperGluePretrainedNetwork.models.utils import frame2tensor, estimate_pose
...
def _generate_lidar_img(
    ...
    pts_camera = pts_camera / pts_camera[:, 2][:, np.newaxis]
    pts_img = (lidar_K@pts_camera.T).T
    # pts_img = pts_img / pts_img[:, 2][:, np.newaxis]
    pts_img = pts_img.astype(int)
    pts_img = pts_img[:, :2]

    # for pt1, pt2 in zip(pts_img, pts):
    #     if pt1[1] == 561 and pt1[0] == 532:
    #         print(pt2)

    # for pt1, pt2 in zip(pts_img, pts):
    #     if pt1[1] == 532 and pt1[0] == 561:
    #         print(pt2)
...
def _init_calib_lidar_to_camera(
    ...
    if len(mkpts0) == 0:
        ...
        for (x0, y0), (x1, y1) in zip(mkpts0, mkpts1):
            cv2.circle(image2, (int(x1), int(y1)), 2, (0, 0, 255), 2)
        cv2.imwrite(save_dir+"/resize-corners.jpg", image2)
        #
        image2 = cv2.imread(str(camera_img_file))
        for (x0, y0), (x1, y1) in zip(mkpts0, mkpts1):
            cv2.circle(image2, (int(x1*2), int(y1*2)), 4, (0, 0, 255), 4)
        cv2.imwrite(save_dir+"/noresize-corners.jpg", image2)
    ...
    for (x0, y0), (x1, y1) in zip(mkpts0, mkpts1):
        x0 = int(x0)
        y0 = int(y0)
        point_index = lidar_index_img[y0, x0]
        if point_index < 0:
            for offset in pick_offsets:
                point_index = lidar_index_img[y0 + offset[0], x0 + offset[1]]
                if point_index >= 0:
                    break
            if point_index < 0:
                continue
        correspondences.append(((x0, y0), (x1, y1)))
        pcd_corners.append(lidar_pts[point_index, :3])
        pcd_cam_corners.append(np.array((x0, y0), dtype=float))
        cam_corners.append(np.array((x1*2, y1*2), dtype=float))
    ...
    result1 = np.hstack((image2, image1))
    for idx, ((x0, y0), (x1, y1)) in enumerate(correspondences):
        result = np.hstack((image2, image1))
        x0 = int(x0)
        x1 = int(x1)
        y0 = int(y0)
        y1 = int(y1)
        cv2.line(result, (x1, y1), (x0 + 1920, y0), color=(0, 0, 255), thickness=1, lineType=cv2.LINE_AA)
        cv2.line(result1, (x1, y1), (x0 + 1920, y0), color=tuple(np.random.randint(0, 255, 3).tolist()), thickness=1, lineType=cv2.LINE_AA)
        cv2.imwrite(save_dir+f"/correspondences_{idx}.jpg", result)
    cv2.imwrite(save_dir+f"/correspondences.jpg", result1)

    pcd_corners = np.array(pcd_corners)
    pcd_cam_corners = np.array(pcd_cam_corners)
    ...
    pcd_save_path = save_dir + "/pcd_corners.yaml"
    pcd_cam_save_path = save_dir + "/pcd_cam_corners.yaml"
    cam_save_path = save_dir + "/img_corners.yaml"

    np_to_cv_yaml.save_corners(pcd_corners, pcd_save_path)
    np_to_cv_yaml.save_corners(pcd_cam_corners, pcd_cam_save_path)
    np_to_cv_yaml.save_corners(cam_corners, cam_save_path)

    logger.info("save corners finished!")
    ...
    T = calib_3d_to_2d(pcd_corners, cam_corners, K)
...
def main():
    ...
    T_lidar2camera = _init_calib_lidar_to_camera(lidar_img_file, camera_img_file, lidar_index_img_file, lidar_pts_file, output_dir)

    #
    img_path = camera_img_file
    pcd_path = "/home/frank/data/G3/at128_cam800/calib_floor.parsed/sensor/lidar_front_at128/1719817220.711602926.pcd"
    K = np.array(
        [
            [2480.296422347829, 0, 1900.995348696508],
            [0, 2483.750770053912, 1103.437012143754],
    ...
    project_pcd_to_img(img_path, pcd_path, T_lidar2camera, K, output_dir)
    pass
